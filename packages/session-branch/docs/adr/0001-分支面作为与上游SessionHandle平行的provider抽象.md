# 0001-分支面作为与上游 SessionHandle 平行的 provider 抽象

上游 `SessionHandle` 模型只有 append-only 面（`create`/`open`/`flush`/
`stat`/`list`），没有显式回退与闭合边界派生原语，且上游 `@deepseek-ai/*`
