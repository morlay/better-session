...
[设计 0001](./.agents/designs/0001-桌面化工具.md)；迁移决策见
[ADR 0003](../../../.agents/adrs/0003-桌面化从golang壳迁移到Electron与上游desktop-host.md)。
...
- **启动即静默退出**（残留 `SingletonLock`，无输出、退出码 0）：清掉 `<userData>/Singleton*` 即可恢复；
  机制见 [设计 0001](./.agents/designs/0001-桌面化工具.md) 的「运行时语义」。
