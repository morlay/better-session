 ...
 *
 * Upstream `apps/desktop-host/src/index.ts` with three local adaptations: the Office composition comes
...
async function main(): Promise<void> {
  ...
  if (process.connected)
    process.send?.(
      { type: "ready", url, injections: ctx.webServer.collectIndexInjections() },
      (error) => {
...
if (import.meta.main) {
  main().catch((error: unknown) => {
    ...
    if (process.connected)
      process.send?.({ type: "fatal", message }, (error) => {
        if (error !== null) console.error(error);
      });
    console.error(error);
    process.exitCode = 1;
