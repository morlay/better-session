...
内置 `@deepseek-ai/dsh-llm-pi-ai` 的 provider profile schema 不含**采样参数**
（`temperature` / `topP` / `topK` / `presencePenalty` / `frequencyPenalty` /
`seed` 均不在 `PiAiProviderProfile` 内），请求级 `GenerateOptions` 之外的采样
默认值无法配置。本插件因此存在：为 OpenAI 兼容端点提供 **profile 级默认采样
参数**（请求级 `GenerateOptions.temperature` 优先），并复用 `llm-pi-ai` 的
dict 多路由结构使现有配置近乎无缝迁移。
...
- 采样默认值合并规则（省略 = 不发送 = 提供方默认）成为本插件的核心差异点
  （规则见 [0006](./0006-采样默认值合并规则与省略语义.md)，现状见
  [README](../../README.md)）。
