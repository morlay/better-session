# @morlay/dsh-client-ui-input-trigger

上游 `@deepseek-ai/dsh-client-ui-input-trigger` 的**整包 fork**（host 半 + client 半），由本仓库维护：
一对一替换官方 `ui-input-trigger` 行（禁用官方行），`InputTriggerSource` / `InputTriggerServiceContract`
只在 `appearance` 词表上扩展。
