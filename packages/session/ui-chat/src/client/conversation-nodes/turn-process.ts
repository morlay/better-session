...
function turnLocation(
  context: ConversationNodeContext<TurnProcessState>,
...
function fallbackState(
  context: ConversationNodeContext<TurnProcessState>,
): TurnProcessState | undefined {
  const turn = context.matches
    .map((match) => eventTurn(match.event))
  ...
  if (evidence.kind === "other") {
    return current.otherStartSeq === undefined
      ? {
          ...
          controlAnchorSeq: Math.min(
            current.controlAnchorSeq ?? Number.POSITIVE_INFINITY,
            evidence.seq,
          ),
        }
      : current;
