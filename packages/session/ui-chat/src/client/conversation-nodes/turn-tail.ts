...
function hasTextAssistant(event: Parameters<ConversationNodeDefinition["match"]>[0]): boolean {
  return (
    ...
    toAssistantBlocks(event.data.message.content).some(
...
function chunkHasText(chunk: StreamChunk): boolean {
  ...
  return (
    chunk.type === "block-end" && chunk.block.type === "text" && chunk.block.text.trim() !== ""
  );
}
...
function tailData(context: ConversationNodeContext<TurnTailState>): TurnTailChatData | null {
  ...
  const finalized = assistants
    .filter(
      (candidate): candidate is Readonly<FinalAssistantChatData> =>
  ...
  for (const match of context.matches) {
    ...
    if (
      candidate !== undefined &&
      (latestTranscriptSeq === undefined || candidate > latestTranscriptSeq)
    ) {
      latestTranscriptSeq = candidate;
    }
  }
  const metrics = deriveTurnMetrics(finalized.map((candidate) => candidate.finalNode)).get(
    end.event.data.turn,
  );
  const tokenUsage =
    context.start?.event.type === "turn/start"
...
export const turnTailDefinition: ConversationNodeDefinition<TurnTailState> = {
  ...
  update: (context, match) =>
    match.event.type === "turn/end" ? { ...context.state, end: match } : context.state,
