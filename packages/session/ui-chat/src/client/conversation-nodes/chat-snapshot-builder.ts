...
class MutableChatNodeStore implements ChatNodeStore {
  ...
  private readonly sources = new Map<
    string,
    MutableChatSource<ChatConversationViewNode | undefined>
  >();
  private readonly processSources = new Map<
    string,
    MutableChatSource<ChatTurnProcessPresentation | undefined>
  >();
  ...
  source(key: string): ChatNodeSource {
    return cachedSource(
      this.sources,
      key,
      () => new MutableChatSource(() => this.get(key), `[ui-chat] node source ${key}`),
    );
  }
  ...
  processSource(key: string): ChatNodeProcessSource {
    return cachedSource(
      this.processSources,
      key,
      () => new MutableChatSource(() => this.process(key), `[ui-chat] node process source ${key}`),
    );
  }
...
class MutableTurnNavigationIndex implements ChatTurnNavigationIndex {
  ...
  rebuild(
    ...
    for (const turn of timeline.turnOrder) {
      ...
      const item =
        previous !== undefined && sameTurnNavigationItem(previous, derived) ? previous : derived;
      next.push(item);
  ...
  touch(turns: ReadonlySet<number>, locations: ChatLocationNodeIndex, nodes: ChatNodeStore): void {
...
function turnProcessPresentations(
  ...
  for (const raw of nodes) {
    ...
    presentations.set(location.turn.turn, {
      ...current,
      earliestProcessAnchor: Math.min(
        current.earliestProcessAnchor ?? node.anchorSeq,
        node.anchorSeq,
      ),
    });
  }
...
export function orderedVisibleChatNodes(
  ...
  return visible.sort((left, right) => {
    ...
    return (
      leftPosition.anchor - rightPosition.anchor ||
...
function sameContribution(
  left: LegacyContribution | undefined,
  right: LegacyContribution,
): boolean {
  return (
    left !== undefined &&
...
class LegacySliceBuilder {
  ...
  private indexContribution(key: string, contribution: LegacyContribution): void {
    updateContributionIndex(
      this.finalizedContributions,
      key,
      contribution,
      contribution.nodes.length > 0,
    );
    updateContributionIndex(
      this.runningContributions,
      key,
      contribution,
      contribution.running !== null,
    );
    updateContributionIndex(
      this.partialContributions,
      key,
      contribution,
      contribution.partial !== null,
    );
  }
  ...
  private rebuildPartial(): void {
    const partial =
      [...this.partialContributions.values()]
        .sort((left, right) => left.anchorSeq - right.anchorSeq)
        .findLast((value) => value.partial !== null)?.partial ?? null;
    if (
      this.partial?.blocks !== partial?.blocks ||
...
function finalizedContributionChanged(
  ...
  return (
    !sameReferences(previousNodes, next.nodes) ||
...
function runningContributionChanged(
  ...
  return (
    ...
    ((previous.running !== null || next.running !== null) && previous.anchorSeq !== next.anchorSeq)
  );
}
...
function partialContributionChanged(
  ...
  return (
    previous?.partial?.blocks !== next.partial?.blocks ||
...
> {
  ...
  apply(input: {
    ...
    for (const node of upserts) {
      ...
      if (
        processPresentationInputChanged(
          previous as ChatNode | undefined,
          node as ChatNode,
          nodeStructural,
        )
      ) {
        const previousTurn =
          previous === undefined ? undefined : locationCoordinates(previous.location).turn;
