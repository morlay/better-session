...
function turnAtLine(list: HTMLElement, line: number): number | null {
  ...
  if (typeof document.elementsFromPoint === "function" && content.width > 0) {
    for (const element of document.elementsFromPoint(content.left + content.width / 2, line)) {
      const row =
        element instanceof HTMLElement ? element.closest<HTMLElement>("[data-chat-turn]") : null;
...
function pagingAnchor(list: HTMLElement, scrollport: HTMLElement): HTMLElement | null {
  ...
  const row = rows[low];
  return row !== undefined && row.getBoundingClientRect().top < visibleBottom
    ? row
    : (rows[0] ?? null);
}
...
function TurnStatus({
  ...
  return (
    <div {...styling.props(styles.turnStatus)} role="status" aria-live="polite">
      {t("chat.deepDiving")}
      {showClock && (
        <span {...styling.props(styles.turnStatusClock)} aria-hidden>
...
const ChatNodeList = memo(function ChatNodeList({ order, ...seatProps }: ChatNodeListProps) {
  return order.map((nodeKey) => <ChatNodeSeat key={nodeKey} nodeKey={nodeKey} {...seatProps} />);
});
...
export function ChatView({
  ...
  const visibleSubmissions = useMemo(() => {
    ...
    return pendingSubmissions.filter(
      (submission) => submission.placement !== "queued" && !observed.has(submission.requestId),
    );
  ...
  const landOnRowRef = useRef<
    (local: HTMLElement, el: HTMLElement, row: HTMLElement, turn: number) => void
  >(() => {});
  ...
  useLayoutEffect(() => {
    ...
    const appendedSteering =
      lastSteeringId !== null && lastSteeringId !== lastSteeringIdRef.current;
    const appendedSubmission =
      lastSubmissionId !== null && lastSubmissionId !== lastSubmissionIdRef.current;
  ...
  useEffect(() => {
    ...
    if (uncovered && hasMore) {
      ...
      if (jumpRepageHeadRef.current !== firstSeq) {
        ...
        void loadThrough(pending.seq).finally(() => {
  ...
  const navigateToTurn = useCallback(
    (item: TurnRailItem): void => {
      ...
      anchorRef.current =
        landed === null || landed.dataset.chatAnchorKey === undefined
          ? null
  ...
  return (
    <div data-chat-view="root" {...styling.props(styles.root)}>
      <div ref={listRef} data-chat-view="scroll" {...styling.props(styles.scroll)}>
        <TurnNavigator
          items={railItems}
          ...
          t={t}
        />
        <div ref={columnRef} {...styling.props(styles.column)} data-chat-flow="">
          {openState === "loading" && (
            <div {...styling.props(styles.hint)}>{t("chat.loadingHistory")}</div>
          )}
          {openState === "error" && openError !== null && (
            <div {...styling.props(styles.openError)}>
              {t("chat.loadError", { message: openError.message, code: openError.code })}
            </div>
          )}
          {hasMore && (
            <div {...styling.props(styles.older)}>
        ...
        {!atBottom && (
          <div data-chat-view="to-bottom" {...styling.props(styles.toBottomSlot)}>
            <button
              type="button"
              {...styling.props(styles.toBottom)}
    ...
    <Modal
      ...
      footer={
        <>
          <Button
            variant="outline"
            className={styling.className(styles.modalAction)}
            onClick={onClose}
          >
            {t("cancel")}
          </Button>
