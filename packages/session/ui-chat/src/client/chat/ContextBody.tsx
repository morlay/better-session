...
import type { KnownContextForm } from "@morlay/dsh-client-ui-conversation/client";
import { styling } from "@morlay/dsh-client-ui-primitives/client";
import { styles } from "./ContextBody.styles.ts";
...
function SourceFields({
  ...
  return (
    <dl {...styling.props(styles.fields)} data-context-fields>
      {rows.map(([key, value]) => (
        <div key={key} {...styling.props(styles.field)}>
          <dt {...styling.props(styles.fieldKey)}>{key}</dt>
          <dd {...styling.props(styles.fieldValue)}>{fieldValue(value, t)}</dd>
        </div>
      ))}
    </dl>
  );
  ...
  return (
    <>
      {contentRuns(content).map((run, index) =>
        "text" in run ? (
          run.text !== "" && (
            <pre key={index} {...styling.props(styles.text)} data-context-text>
...
export function InstructionsBody({
  ...
  return (
    <>
      <ul {...styling.props(styles.files)} data-context-files>
        {changes.map((change) => (
          <li key={change.path} {...styling.props(styles.file)} title={change.digest}>
            <span {...styling.props(styles.filePath)}>{change.path}</span>
            <span {...styling.props(styles.fileAction)}>
...
export function CatalogBody({
  ...
  return (
    <>
      {update && (
        <p {...styling.props(styles.catalogNotice)} data-context-catalog-update>
          {t("message.context.catalog.replaced")}
        </p>
      )}
      <ul {...styling.props(styles.entries)} data-context-entries>
        {shown.map((entry, index) => (
          // Index key: a hand-edited or foreign log may repeat a name, and a
          // duplicate React key would drop a row the model did see.
          <li key={index} {...styling.props(styles.entry)}>
            <code {...styling.props(styles.entryName)}>{entry.name}</code>
            <span {...styling.props(styles.entryDescription)}>{entry.description}</span>
          </li>
        ))}
      </ul>
      {shown.length < entries.length && (
        <p {...styling.props(styles.catalogNotice)} data-context-entries-truncated>
...
export function SnapshotBody({
  ...
  return (
    <>
      <p {...styling.props(styles.catalogNotice)} data-context-snapshot-supersedes>
        {t("message.context.snapshot.supersedes")}
      </p>
      <dl {...styling.props(styles.sections)} data-context-sections>
        {sections.map((section, index) => (
          <div key={index} {...styling.props(styles.section)}>
            <dt {...styling.props(styles.sectionName)}>{section.name}</dt>
            <dd {...styling.props(styles.sectionText)}>{boundedText(section.text, t)}</dd>
          </div>
        ))}
      </dl>
    </>
...
export function RelayBody({
  ...
  return (
    <>
      <p {...styling.props(styles.relaySender)} data-context-relay-sender>
...
export function RecallBody({
  ...
  return (
    <>
      <ul {...styling.props(styles.recalls)} data-context-recalls>
        {sessions.map((session, index) => (
          <li key={index} {...styling.props(styles.recall)}>
            <span {...styling.props(styles.recallLabel)}>{session.label}</span>
            <span {...styling.props(styles.recallCounts)}>
            ...
            {session.truncated && (
              <span {...styling.props(styles.recallCounts)}>
                {t("message.context.recall.truncated")}
              </span>
            )}
          </li>
        ))}
