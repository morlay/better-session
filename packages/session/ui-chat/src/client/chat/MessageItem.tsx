...
import type { MessageImageSource } from "@morlay/dsh-client-ui-conversation/client";
import {
  fileExtension,
  FileTypeIcon,
  fileSizeText,
...
import { ReferenceMarkdown, styling } from "@morlay/dsh-client-ui-primitives/client";
...
function contentParts(content: readonly unknown[]): {
  ...
  return { blocks, attachments, rest };
}
...
function ModelRetryItem({
  ...
  return (
    <details {...styling.props(styles.retryRow)} data-active={active || undefined}>
      <summary {...styling.props(styles.retrySummary)} data-retry-summary="">
        <span {...styling.props(styles.retryText)} data-retry-text="" role="status">
          {t("message.retry.status", { label, retry: node.retry, maximum, seconds })}
        </span>
      </summary>
      <div {...styling.props(styles.retryDetails)}>
        <div>
          <span {...styling.props(styles.retryDetailLabel)}>{t("message.retry.delay")}</span>
          {t("duration.milliseconds", { milliseconds: Math.round(node.delayMs) })}
        </div>
        <div>
          <span {...styling.props(styles.retryDetailLabel)}>{t("message.retry.failure")}</span>
...
function TurnErrorItem({ node, t }: { node: TurnErrorNode; t: ChatViewSlotProps["t"] }) {
  return (
    <div {...styling.props(styles.turnErrorRow)} role="status">
      <StateDot state="error" className={styling.className(styles.turnErrorDot)} />
      <div {...styling.props(styles.turnErrorCopy)}>
        <span {...styling.props(styles.turnErrorTitle)}>{t("message.turnError")}</span>
        <span {...styling.props(styles.turnErrorMessage)}>
          {failureMessage(node.message, node.code, t)}
        </span>
      </div>
      {node.code !== undefined && <code {...styling.props(styles.turnErrorCode)}>{node.code}</code>}
    </div>
  );
}
...
function TurnMaxTokensItem({ t }: { t: ChatViewSlotProps["t"] }) {
  return (
    <div {...styling.props(styles.turnErrorRow)} role="status">
      <StateDot state="warning" className={styling.className(styles.turnErrorDot)} />
      <div {...styling.props(styles.turnErrorCopy)}>
        <span {...styling.props(styles.maxTokensTitle)}>{t("message.maxTokens")}</span>
        <span {...styling.props(styles.turnErrorMessage)}>{t("message.maxTokens.hint")}</span>
      </div>
    </div>
  );
}
...
function UserStyleBubble({
  ...
  echo?: boolean;
  /** Exact session mention labels associated by the adjacent recall node (now rendered by markdown). */
  referenceLabels?: readonly string[];
  /** Skill names the step's `skill-invocation` injections loaded for this message (now rendered by markdown). */
  ...
  const { blocks, attachments: contentAttachments, rest } = contentParts(content);
  // 多块消息按空行拼回 raw markdown（新式提交就是单块原文）。
  const text = blocks.join("\n\n");
  // labels 按 locale revision 稳定（官方 MarkdownText 在它上面 memo 渲染缓存）。
  const labels = useMemo(() => markdownLabels(t), [t]);
  // 引用 chip 的点击目标；ReferenceMarkdown 内部把它接进官方 fileMentions。
  const mentionActions = useMemo(
    () =>
      references === undefined
        ? undefined
        : { openFile: references.openFile, openSkill: references.openSkill },
    [references],
  );
      ...
      <div {...styling.props(styles.userStack)}>
          ...
          <div {...styling.props(styles.attachmentRow)} data-message-attachments>
            {attachments.map((attachment, index) =>
              attachment.type === "image" ? (
                ...
                <span
                  key={`file:${index}`}
                  {...styling.props(styles.fileCard)}
                  title={attachment.file.name}
                >
                  <FileTypeIcon
                    path={attachment.file.name}
                    className={styling.className(styles.fileIcon)}
                  />
                  <span {...styling.props(styles.fileContent)}>
                    <span {...styling.props(styles.fileName)}>{attachment.file.name}</span>
                    <span {...styling.props(styles.fileMeta)}>
                      {[
        ...
        {showBubble && (
          <div {...styling.props(styles.bubble)}>
            <ReferenceMarkdown text={text} labels={labels} actions={mentionActions} />
            {rest.map((block, i) => (
              <JsonBlock
                key={i}
 ...
 */
export function PendingSubmissionBubble({
  submission,
  renderMessageImages,
  t,
}: {
          ...
          ? {
              ...
              image: {
                preview: {
                  ...
                  ...(attachment.value.width === undefined
                    ? {}
                    : { width: attachment.value.width }),
                  ...(attachment.value.height === undefined
                    ? {}
                    : { height: attachment.value.height }),
                },
              },
            }
...
export const UnknownNodeView = memo(function UnknownNodeView({
  ...
  const data = node.data;
  return (
    <div {...styling.props(styles.contextRow)}>
      <JsonBlock
