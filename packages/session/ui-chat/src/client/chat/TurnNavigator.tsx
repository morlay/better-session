...
import { styles } from "./TurnNavigator.styles.ts";
...
function TurnNavigatorRail({ items, activeTurn, busyTurn, onNavigate, t }: TurnNavigatorProps) {
  ...
  const previewAtPointer = (event: PointerEvent<HTMLElement>): void => {
    ...
    setPreviewTurn(
      itemAtPointer(items, event.currentTarget, scrollTop, event.clientY)?.turn ?? null,
    );
  };
  ...
  // 原先靠类名数组拼接（.fadeTop.fadeBottom 靠两个类同时存在生效），
  // 现在由 styling.className 合并成一条规则，同时把该组合显式带上。
  const fadeClass = styling.className(
    styles.scroller,
    scrollState.canScrollUp && styles.fadeTop,
    scrollState.canScrollDown && styles.fadeBottom,
    scrollState.canScrollUp && scrollState.canScrollDown && styles.fadeTopFadeBottom,
  );
  return (
    <div {...styling.props(styles.slot)}>
      <nav
        {...styling.props(styles.frame)}
      ...
      >
        ...
        >
          <div {...styling.props(styles.marks)}>
            {items.map((item, index) => {
              const active = item.turn === activeTurn;
              const showingPreview = item.turn === previewTurn;
              const markClass = styling.className(
                styles.mark,
                item.anchor.kind === "unloaded" && styles.markUnloaded,
                active ? styles.markActive : showingPreview && styles.markPreview,
                item.turn === busyTurn && styles.markBusy,
              );
              return (
                <div
                  key={item.turn}
                  {...styling.props(styles.markPosition)}
        ...
        {preview !== undefined && previewPosition !== undefined && (
          <div
            id={previewId}
            role="tooltip"
            {...styling.props(styles.preview)}
            style={previewPosition}
          >
            <div {...styling.props(styles.previewPrompt)}>
              {preview.prompt || t("chat.turnNavigation.turn", { turn: preview.turn })}
            </div>
            {preview.response !== "" && (
              <div {...styling.props(styles.previewResponse)}>{preview.response}</div>
            )}
          </div>
        )}
      </nav>
