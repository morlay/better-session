...
import { styles } from "./GenericCommandCard.styles.ts";
...
export function GenericCommandCard({ node, t, runningSummary }: GenericCommandCardProps) {
  ...
  return (
    <div {...styling.props(styles.root)} data-variant="others" data-state={state}>
      {state === "running" && (
        <span {...styling.props(a11yStyles.visuallyHidden)}>{t("row.running")}</span>
      )}
      {state === "error" && (
        <span {...styling.props(a11yStyles.visuallyHidden)}>{t("row.failed")}</span>
      )}
      <DisclosureRow
        ...
        collapsedContent={
          <>
            ...
            <span {...styling.props(styles.summary)} data-error={state === "error" || undefined}>
              {summary}
            </span>
          </>
        }
      >
        <pre {...styling.props(styles.body)} data-error={state === "error" || undefined}>
          {body}
        </pre>
      </DisclosureRow>
    </div>
