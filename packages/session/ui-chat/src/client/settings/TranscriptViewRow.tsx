...
import { styles } from "./TranscriptViewRow.styles.ts";
...
export function TranscriptViewRow({
  ...
  const selectedLabel =
    mode === "normal" ? "settings.transcript.normal" : "settings.transcript.compact";
  const closeMenu = () => {
    setOpen(false);
  };
  ...
  const selector = (
    ...
    >
      {t(selectedLabel)}
      <IconChevronDownOutline14 className={styling.className(styles.chevron)} />
    </button>
  );
  ...
  return (
    <div {...styling.props(styles.row)}>
      <div {...styling.props(styles.rowText)}>
        <div {...styling.props(styles.title)}>{t("settings.transcript.title")}</div>
        <div {...styling.props(styles.desc)}>{t("settings.transcript.description")}</div>
      </div>
      <Menu
        open={open}
        onClose={closeMenu}
