...
import {
  ...
  type ConversationLocationDataChange,
...
function emptyLocationData(): Record<
  ConversationLocationDataScope,
...
export class ConversationNodeAssembler implements ConversationViewSnapshotStore {
  ...
  settleAssistant(
    ...
    this.revised.clear();
    const retired = [...this.inputs.values()].filter(
      (candidate): candidate is SessionTransientEventEntry =>
    ...
    for (const [key, entries] of pending) {
      ...
      const additions = entries
        .map((entry) => {
          ...
          const owners =
            this.contextsBySeq.get(entry.match.event.seq) ?? new Set<InternalContext>();
          owners.add(context);
  ...
  private indexStartedContexts(kind: string, additions: readonly InternalContext[]): void {
    ...
    while (before < existing.length || added < sorted.length) {
      ...
      if (
        right === undefined ||
        (left !== undefined && (left.startSeq as number) < (right.startSeq as number))
      ) {
...
function isLocationBoundary(type: string): boolean {
  return (
    type === "turn/start" || type === "turn/end" || type === "step/start" || type === "step/end"
  );
}
