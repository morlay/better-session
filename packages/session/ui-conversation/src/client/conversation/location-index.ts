...
export class ConversationLocationIndex {
  ...
  replaceData(
    entries: readonly { readonly owner: string; readonly data: ConversationLocationData }[],
  ): boolean {
      ...
      const values =
        data.kind === "turn"
          ...
          : (steps.get(stepDataKey(data.turn, requireStep(data))) ??
            new Map<string, OwnedLocationData>());
  ...
  rebuild(entries: readonly SessionEventLikeEntry[]): ReadonlySet<number> {
    ...
    for (const { event } of entries) {
      ...
      const step =
        explicit.session === true || event.type === "turn/start" || event.type === "turn/end"
          ? undefined
  ...
  private stepData(
    ...
    return this.mutableStepData(
      stepDataKey(turn, step),
    ) as ConversationLocationDataStore<ConversationStepDataMap>;
  }
