...
import { styles } from "./EnterBehaviorRow.styles.ts";
...
export type EnterBehaviorRowProps = PropsRuntime<"settings.general.item"> &
  ...
  return (
    <div {...styling.props(styles.row)}>
      <div {...styling.props(styles.rowText)}>
        <div {...styling.props(styles.title)}>{t("settings.enter.title")}</div>
        <div {...styling.props(styles.desc)}>{t("settings.enter.description")}</div>
      </div>
      <Menu
        open={open}
        onClose={() => {
