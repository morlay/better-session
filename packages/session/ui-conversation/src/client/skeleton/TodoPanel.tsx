...
import { styles } from "./TodoPanel.styles.ts";
...
function CompletedGlyph() {
  return (
    <svg
      ...
      fill="none"
      aria-hidden="true"
      {...styling.props(styles.glyphCompleted)}
    >
...
function ProgressGlyph() {
  ...
  return (
    <svg
      ...
      fill="none"
      aria-hidden="true"
      {...styling.props(styles.glyphProgress)}
    >
      <defs>
        <linearGradient
          id={gradientId}
...
function PendingGlyph() {
  return (
    <svg
      ...
      fill="none"
      aria-hidden="true"
      {...styling.props(styles.glyphPending)}
    >
      <circle
        cx="7"
        cy="7"
...
export function TodoPanel({ todos, t }: TodoPanelProps) {
  ...
  return (
    <section {...styling.props(styles.root)} data-testid="todo-panel" aria-label={t("todo.title")}>
      <div {...styling.props(styles.body)}>
        ...
        >
          ...
          <span {...styling.props(styles.title)}>{t("todo.title")}</span>
          <span {...styling.props(styles.progress)}>{progressLabel(todos, t)}</span>
        ...
        {!collapsed && (
          <ul {...styling.props(styles.list)}>
            {todos.map((item) => (
              <li key={item.content} {...styling.props(styles.item)} data-status={item.status}>
                <span {...styling.props(styles.glyph)} aria-hidden>
                  <StatusGlyph status={item.status} />
                </span>
                <span {...styling.props(styles.content)}>{item.content}</span>
              </li>
            ))}
          </ul>
        )}
