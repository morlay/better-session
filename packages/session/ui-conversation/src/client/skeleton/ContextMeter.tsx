...
import { styles } from "./ContextMeter.styles.ts";
...
const ROWS = [
  { key: "systemTokens", label: "context.system", color: styling.className(styles.colorSystem) },
  { key: "toolsTokens", label: "context.tools", color: styling.className(styles.colorTools) },
  {
    key: "messageTokens",
    label: "context.messages",
    color: styling.className(styles.colorMessages),
  },
] as const;
...
export function ContextMeter({ useProjection, t }: ContextMeterProps) {
  ...
  return (
    <span ref={rootRef} {...styling.props(styles.root)}>
      <Tooltip
      ...
      >
        ...
        >
          <svg viewBox="0 0 14 14" width="14" height="14" aria-hidden>
            <circle {...styling.props(styles.track)} cx="7" cy="7" r={RADIUS} />
            <circle
              {...styling.props(styles.fill)}
              cx="7"
              cy="7"
              r={RADIUS}
      ...
      {open && (
        <div {...styling.props(styles.panel)} role="dialog" aria-label={t("context.used")}>
          <div {...styling.props(styles.header)} data-conversation-header="">
            {/* Empty sides collapse through `.headline:empty` so the locale that
                needs no leading (or trailing) text spends no header gap. */}
            <span {...styling.props(styles.headline)}>{headBefore}</span>
            <span {...styling.props(styles.percent)}>{reading}</span>
            <span {...styling.props(styles.headline)}>{headAfter}</span>
            {/* `~`: usedTokens prefers projectedTokens, whose surface delta is
                heuristically repriced on top of the provider-anchored sample. */}
            <span {...styling.props(styles.figures)}>
              {`~${formatTokens(context.usedTokens, t)} / ${formatTokens(context.contextWindow, t)}`}
            </span>
          </div>
          <div {...styling.props(styles.bar)}>
            {segments.map((segment) => (
              <div
                key={segment.key}
                className={
                  segment.color === undefined
                    ? styling.className(styles.segment)
                    : `${styling.className(styles.segment)} ${segment.color}`
                }
          ...
          {breakdown !== undefined && (
            <dl {...styling.props(styles.rows)}>
              {ROWS.map((row) => (
                <div key={row.key} {...styling.props(styles.row)}>
                  <dt>
                    <span
                      className={`${styling.className(styles.swatch)} ${row.color}`}
                      aria-hidden
                    />
                    {t(row.label)}
                  </dt>
