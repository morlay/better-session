...
import { styles } from "./HeroShell.styles.ts";
...
export function WorkspaceChip({
  ...
  return (
    <button
      ref={buttonRef}
      type="button"
      {...styling.props(styles.workspace)}
    ...
    >
      {label === undefined ? (
        <IconFolderClose16 className={styling.className(styles.folder)} size={16} />
      ) : (
        <IconFolderOpen16 className={styling.className(styles.folder)} size={16} />
      )}
      <span {...styling.props(styles.workspaceLabel)}>{label ?? t("hero.chooseWorkspace")}</span>
      <IconChevronDownOutline14 className={styling.className(styles.chevron)} size={12} />
    </button>
  );
}
...
export function HeroShell({ t, renderSlot, children }: HeroShellProps) {
  ...
  return (
    <div {...styling.props(styles.root)}>
      <div {...styling.props(styles.stack)}>
        <div {...styling.props(styles.headline)}>
          {/* figma 34:10412: fish 34×25 leading the headline, gap 10. */}
          <span
            {...styling.props(styles.fishHitbox)}
            onMouseEnter={() => {
              if (
          ...
          >
            {renderSlot(
              ...
              { size: 34, className: `${styling.className(styles.fish)} dsh-hero-fish` },
              {
                fallback: <HeroFish hovering={hovering} />,
              },
            )}
          </span>
          <span {...styling.props(styles.titleGroup)}>
            {/* Own element: keeps the headline text addressable apart from the badge. */}
            <span>{t("hero.headline")}</span>
            <span {...styling.props(styles.previewBadge)}>{t("hero.preview")}</span>
          </span>
        </div>
        <div {...styling.props(styles.body)}>
