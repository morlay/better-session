# 0008-跟随上游 Session format v3 的读取与迁移适配

状态：已采纳

上游 0.1.5-alpha.1 把 `SESSION_FORMAT_VERSION` 从 2 升到 3，并改了会话日志的
...
- v2 会话在首次写打开时整体重写落库（`rewriteMigratedLog`），读坐标包含
  迁移注入的 system head（见 [旧数据修复](../designs/0008-旧数据修复.md)）。
