...
状态：已实现（坐标模型与核心不变量已落地）
范围：本包（`packages/session/session-rdb/`）的设计总览、坐标模型与导航；各主题细节见 0002–0008。
...
├── .agents/                     # 本包记录树（设计与决策各一个 home）
│   ├── designs/                 # 设计：0001 总览 / 0002 分支 / 0003 表结构 / 0004 写路径 /
│   │                            #   0005 读路径 / 0006 并发写入 / 0007 事件重建 / 0008 旧数据修复
│   └── adrs/                    # 决策记录（0001–0011）
...
3. **fork 坐标自洽**：fork 前缀经 `readLog` 读取视图（replace range 已是稠密
   ...
   两段空间重叠）（见 [分支能力](0002-分支能力.md)）。
4. **rewind 保留区 range 完整性**：replace range 引用更早事件 ⇒ 截断尾部
   不破坏保留区 range（见 [分支能力](0002-分支能力.md)）。
5. **SCHEMA_VERSION 门禁**：破坏性表结构变更必须 bump；表结构级升级走
   ...
   见 [读路径](0005-读路径.md) / [旧数据修复](0008-旧数据修复.md)）。

## 文档导航

| 主题                                       | 文档                             |
| ------------------------------------------ | -------------------------------- |
| 数据表设计（元数据 / 事件实体 / 会话桥接） | [表结构](0003-表结构.md)         |
| 写路径流程（appendBatch）                  | [写路径](0004-写路径.md)         |
| 读路径流程（load / readFrom）              | [读路径](0005-读路径.md)         |
| 分支能力（forkFrom / rewind / timeline）   | [分支能力](0002-分支能力.md)     |
| 旧数据修复（读取/导出视图修复）            | [旧数据修复](0008-旧数据修复.md) |
| 并发写入者检测                             | [并发写入](0006-并发写入.md)     |
| 事件 id 关联与重建可行性分析               | [事件重建](0007-事件重建.md)     |
