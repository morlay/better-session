...
状态：已实现（forkFrom / rewind / timeline 均已落地）
...
- **事件行复用**：`t_events` 是全局实体，派生会话的桥接行直接引用源会话
  ...
  **延续**（`f_data` 完整保留 turn/step，复用事件行即延续），子会话后续 append
  从边界处继续编号（复用机制见 [写路径](0004-写路径.md)）；
...
停止端口（[编排层 ADR 0005](../../../ui-conversation-message-actions/.agents/adrs/0005-rewind前主动停止运行中的loop.md)）。
...
[ADR 0005](../adrs/0005-未闭合轮次原样保留补closers由消费方负责.md)），只对
