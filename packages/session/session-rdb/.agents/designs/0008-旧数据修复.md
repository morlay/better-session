...
状态：已实现
范围：旧数据的读取 / 导出视图修复与写打开时的落库迁移；表结构级迁移见 0003。
...
表结构版本策略、迁移执行方式与 v3 diff 内容见 [表结构](0003-表结构.md)；
本节只讲数据格式级修复。
...
5. 交给读取视图修复（`repairReadView`）：补 `assistant/message.stream`、越界
   ...
   孤儿 inbox splice 改写（见 [读路径](0005-读路径.md)）。
...
容错修复（明细见 [读路径](0005-读路径.md)，含继承前缀收缩）→ 重算 replace
...
根因与修复见 [分支能力](0002-分支能力.md)：rewind 截断后在事务内把 `f_seed_length`
