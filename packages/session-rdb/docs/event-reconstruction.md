...
| compaction 后重放                              | ✅           | `shadowedRange` 与紧随 replace range 同空间（原样坐标），"no adjacent shadow price" 契约成立                                                                                   |
...
| 导出（readRaw）后重放                          | ✅           | 导出即修复：序列化前修复 surface 语义（非法 replace 降级 append）并重算 provenance，artifact 完备可用——导入后无需任何修复即可加载（见 [legacy-clean.md](legacy-clean.md)）     |
