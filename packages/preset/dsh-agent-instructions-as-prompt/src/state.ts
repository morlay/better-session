...
function versionStatesFor(
  session: Session,
  cache: InstructionVersionCache,
): Map<string, InstructionVersionState> {
...
export function retainedInstructionVersionUpdates(
  ...
  return updates.filter((update) =>
    renderedChanges.some((change) => sameInstructionChange(update.change, change)),
  );
}
...
export async function reconcileInstructionContext(
  ...
  const pushRemoval = (scope: string, path: string): void => {
    ...
    items.push({
      change,
      file: { absolutePath: `removed:${scope}`, displayPath: path, content: "" },
    });
  ...
  for (const [directory, directoryScopes] of scopesByDirectory) {
    ...
    for (const scope of probedScopes) {
      const previous = effective.get(scope);
      const probe = await probeScopeInstruction(
        scope,
        projectRoot,
        resolved,
        fileSystem,
      ...
      if (
        previous !== undefined &&
        previous.action !== "remove" &&
        previous.path === file.displayPath &&
