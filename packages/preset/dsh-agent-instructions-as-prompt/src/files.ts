 ...
 * (upstream `packages/context/agent-instructions/src/`); local changes live in
 * `./index.ts`, `./state.ts`, `./render.ts`, `./prompt.ts`, `./section-marker.ts`.
 */
...
async function statFile(
  ...
  return fileSystem === undefined
    ? nodeStatFile(path, signal)
    : fsStatFile(path, fileSystem, signal);
}

async function existsAsMarker(
  path: string,
  fileSystem?: FileSystem,
  signal?: AbortSignal,
): Promise<boolean> {
...
export function descendantDirsBetween(root: string, touchedPath: string): string[] {
  const resolvedRoot = resolve(root);
  const targetPath = isAbsolute(touchedPath)
    ? resolve(touchedPath)
    : resolve(resolvedRoot, touchedPath);
 ...
 */
export async function discoverBaselineInstructionFiles(
  options: DiscoverOptions,
...
async function readBounded(
  ...
  try {
    const chunks =
      fileSystem === undefined || file.target === undefined
 ...
 */
export function dedupInstructionFilesByDirectory(
...
export async function probeScopeInstruction(
  ...
  const dir =
    directory === USER_GLOBAL_DIRECTORY
      ? resolved.dshHome
      : directory === "."
        ? projectRoot
