...
function buildInstructionText(
  ...
  const body = [marker, style.intro, ...files.map((file) => style.section(file))].filter(
...
function renderInstructionContext(
  ...
  for (let start = 1; start < files.length; start += 1) {
    ...
    const omitted = files
      .slice(0, start)
      .map((file) => ({ absolutePath: file.absolutePath, displayPath: file.displayPath }));
  ...
  const text =
    byteLength(compactNotice) <= maxBytes ? compactNotice : truncateUtf8(compactNotice, maxBytes);
  return { text, omitted, truncated, represented: [] };
}
