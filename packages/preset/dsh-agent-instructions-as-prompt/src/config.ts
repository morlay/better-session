...
export const Config: z<Config> = z.object({
  ...
  localInstructionFileCandidates: z
    .array(z.string())
    .default([...DEFAULT_LOCAL_INSTRUCTION_FILE_CANDIDATES]),
});
...
function resolveInstructionFileCandidates(
  candidates: string[] | undefined,
  fallback: readonly string[],
): string[] {
  return (candidates ?? [...fallback]).filter(
    (candidate) => !RESERVED_PATH_SEGMENTS.has(candidate) && !/[\\/]/.test(candidate),
  );
}
