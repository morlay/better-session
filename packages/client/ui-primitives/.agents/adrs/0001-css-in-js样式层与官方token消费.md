# 0001-css-in-js 样式层与官方 token 消费

状态：已采纳

fork 过来的对话 UI（`packages/session/ui-{conversation,chat,input-trigger}`）沿用上游的 CSS Modules：
...
token 树由`packages/client/ui-primitives/scripts/gen-design-tokens.mts` 生成（357 个 `--dsw-*`），`pnpm --filter
@morlay/dsh-client-ui-primitives run gen:tokens` 重新生成；`design-tokens.spec.ts` 守卫漂移
（树与上游定义集合逐一相等）、往返（叶子能还原变量名）与默认值完整性。
...
| 手写一份常用 token 清单                                  | 361 个变量里跟不住上游增删，漂移无人守                                                            |
...
- 后续：fork 包逐个把 CSS Modules 换成 `styled` + `dsw`（属阶段 1 起的实施内容）。
