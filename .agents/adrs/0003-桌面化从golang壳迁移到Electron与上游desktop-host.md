...
上游 `vendor/deepseek-harness/apps/desktop`（Electron 壳）与
`vendor/deepseek-harness/apps/desktop-host`（字节管道后端）
...
- **壳**：Electron 主进程，复用上游 `host-process.ts` / `host-protocol.ts`
  （复制到 `packages/desktop/dsh-desktopify/src/`，纯 node 无 electron 依赖）。
...
- **工具形态**：`@morlay/dsh-desktopify` 提供 bin `dsh-desktopify`
  （对外子命令只有 `dev` / `bundle`；`prepare:runtime` / `prepare:seed` 是内部
  模块，不单独暴露），
